...
class StatusLengthValidator < ActiveModel::Validator
  ...
  def validate(status)
    ...
    status.errors.add(:text, I18n.t('statuses.over_character_limit', max: Setting.max_toot_chars.to_i)) if too_long?(status)
  end

  private

  def too_long?(status)
    countable_length(combined_text(status)) > Setting.max_toot_chars.to_i
  end
